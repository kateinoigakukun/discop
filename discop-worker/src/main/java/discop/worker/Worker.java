...
public class Worker {
    public void run(ClusterMessage.RunJob message) {
        try (Store store = new Store();
             Engine engine = store.engine();
             Module module = Module.fromBinary(engine, message.getWasiBytes().toByteArray());
    ...
    public void experimentalRunJob(SchedulerMessage.Job message) {
        try (Store store = new Store();
             Engine engine = store.engine();
